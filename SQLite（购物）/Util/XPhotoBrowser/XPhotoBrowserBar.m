...
//    _bgView.backgroundColor = [UIColor colorWithRed:0x01/255.0f green:0xbb/255.0f blue:0x91/255.0f alpha:1.0f];
    ...
    //
    self.leftButton = [self createButton:CGRectMake(10, self.frame.size.height - 44 - 10, 44, 44) withNormalStateImage:[UIImage imageNamed:@"all_back.png"] withTarget:self withAction:@selector(leftButtonDidTouch:) isleft:YES];
    [self addSubview:self.leftButton];
    
//    self.rightButton = [self createButton:CGRectMake(self.frame.size.width - 44 - 10, self.frame.size.height - 44 - 10, 44, 44) withNormalStateImage:[UIImage imageNamed:@"cart_delete.png"] withTarget:self withAction:@selector(rightButtonDidTouch:) isleft:NO];
